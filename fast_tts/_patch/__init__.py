...
keeps plain ``qwen-tts`` installable alongside this one. If a future upstream
release renames any API the overlay depends on, :mod:`fast_tts._compat` fails
fast at import/load time instead of letting streaming calls crash later.
"""
...
def apply_patch() -> None:
    ...
    from .._compat import check_qwen_tts_version, probe_model_class
    ...
    # Fail fast on qwen-tts API drift before attaching anything.
    check_qwen_tts_version()
    probe_model_class(stock_cls)
