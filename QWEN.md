...
- flash-attn: built locally in `G:\qwen-tts\flash-attention\`

### Flash Attention Build Fix (CUDA 13.2 + MSVC 14.43)
When building flash-attn, add to `setup.py`:
- `/Zc:preprocessor` compiler flag
- `-DCCCL_IGNORE_MSVC_TRADITIONAL_PREPROCESSOR_WARNING` feature flag
...
- `project/cuda13-msvc-build.md` — flash-attn build fix
